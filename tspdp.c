...
typedef struct term aterm; //what is this for?
...
int gis(int a, int s[10], int m1, int v[10])
{
  ...
  for(k=1;k<=n;k++) visited[k]=v[k];
  visited[i]=1;  // last visited city, true assignment
  if(m==1) return(cost[i][s[1]] + cost[s[1]][1]); // terminating condition
...
user@user-Inspiron-3558:~$ gcc tspdf.c
user@user-Inspiron-3558:~$ ./a.out
...
shortest tour for Travelling Salesman is
1-->2-->4-->3-->1
min dist is 35
user@user-Inspiron-3558:~$ 
